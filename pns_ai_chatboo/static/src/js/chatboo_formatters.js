odoo.define('pns_ai_chatboo.formatters', function (require) {
    "use strict";

    /**
     * pns_ai_chatboo.formatters — Pure formatting utilities for Chatboo.
     *
     * Extracted from ChatbooComponent to enable:
     *   - Independent testing
     *   - Reuse across components (floating chat, full action, future OWL 2)
     *   - Smaller main component file
     *
     * All functions are pure (no `this`, no state mutation).
     * Internal cross-references use the module-level function name directly.
     *
    ...
    function getSessionLocale() {
        ...
        try {
            if (typeof odoo !== 'undefined' && odoo.session_info) {
                var uc = odoo.session_info.user_context || {};
                lang = uc.lang || odoo.session_info.user_lang || odoo.session_info.lang || lang;
    ...
    function getSessionTz() {
        try {
            if (typeof odoo !== 'undefined' && odoo.session_info) {
                var uc = odoo.session_info.user_context || {};
                return (uc.tz || odoo.session_info.tz || '').trim();
            }
        } catch (e) { /* keep empty */ }
        return '';
    }
    ...
    function formatContent(content) {
        ...
        if (typeof ChatbooSse !== 'undefined' && ChatbooSse.splitHtmlAndMarkdownTail) {
            ...
            if (split) {
                ...
                return _wrapStandaloneImages(enhanceHtmlProse(ChatbooSse.mergeHtmlWithFooter(split.html, foot)));
            }
        }
    ...
    return {
        ...
        getSessionTz: getSessionTz,
        formatWallclock: formatWallclock,
        isLikelyHtml: isLikelyHtml,
        escapeHtml: escapeHtml,
        formatContent: formatContent,
        formatMarkdown: formatMarkdown,
        enhanceHtmlProse: enhanceHtmlProse,
        formatCSV: formatCSV,
        formatJsonWithLineBreaks: formatJsonWithLineBreaks,
        formatNumber: formatNumber,
        formatJsonAsTable: formatJsonAsTable,
    };
});
