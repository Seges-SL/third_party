odoo.define('pns_ai_chatboo.export', function (require) {
    "use strict";
     ...
     *
     * Extracted from ChatbooComponent. Functions that need component state
     * receive an `ctx` object with the required dependencies:
     *
     *   @typedef {Object} ExportContext
     *   @property {Array}    messages     - Component message array (read-only)
     *   @property {Element}  messagesEl   - messagesRef.el DOM node (read-only)
     *   @property {Function} notification - props.notification toast function
     *
     * Pure utility functions (htmlToMarkdown, tableToMarkdown, etc.) need no ctx.
     *
     * OWL 2 migration: change `odoo.define(... return {})` to `export { ... }`.
     */
    ...
    function copyToClipboard(ev, ctx) {
        ...
        if (copyType === 'markdown' && msg.role === 'assistant') {
            ...
            if (plainHtml) {
                ...
                var messageCards = ctx.messagesEl.querySelectorAll('.o_chatboo_message');
    ...
    return {
        ...
        markdownToPDFText: markdownToPDFText,
        markdownToHTML: markdownToHTML,
        normalizeFilename: normalizeFilename,
        extractPlainText: extractPlainText,
        generateFilename: generateFilename,
        sessionFileHref: sessionFileHref,
        sessionFileIsInline: sessionFileIsInline,
        doCopy: doCopy,
        ...
        ensureChartPngs: ensureChartPngs,
        fulfillPendingSessionDocuments: fulfillPendingSessionDocuments,
    };
});
