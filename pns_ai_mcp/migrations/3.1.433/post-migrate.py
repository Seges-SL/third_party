...
"""v3.1.433: reset leftover composition_origin token translations (Native→Nativo)."""
import logging
...
def migrate(cr, version):
    if _table_exists(cr, 'ir_translation'):
        ...
        cr.execute(
            ...
            """,
            (_OLD_LABELS, '%composition_origin%'),
        )
        _logger.info(
            'pns_ai_mcp 3.1.433: reset composition_origin token translations',
        )
    env = api.Environment(cr, SUPERUSER_ID, {})
    if 'ir.model.fields.selection' not in env:
        return
