    ...
    def generate_onedrive_refresh_token(self):
        ...
        try:
            ...
            if res.status_code != 200:
                raise ValidationError("Bad microsoft onedrive request..!")
                    ...
                    if rec.auto_remove:
                        ...
                        for f in files:
                            create_time = datetime.datetime.strptime(
                                ftp_server.sendcmd('MDTM ' + f)[4:],
                                "%Y%m%d%H%M%S")
                            diff_days = (
                                    datetime.datetime.now() - create_time).days
                ...
                try:
                    ...
                    if rec.auto_remove:
                        ...
                        expired = list(filter(lambda fl: (
                                                             ...
                                                             sftp.stat(
                                                                 fl).st_mtime)).days >= rec.days_to_remove,
                                              files))
                        for file in expired:
                            sftp.unlink(file)
                    sftp.close()
                ...
                try:
                    ...
                    try:
                        upload_session = requests.post(
                            upload_session_url,
                            headers=headers)
                        upload_url = upload_session.json().get(
                            'uploadUrl')
